    ...
    <div className="min-h-screen pb-20 selection:bg-indigo-100">
      ...
      <main className="max-w-7xl mx-auto px-4 mt-10 md:mt-14">
        ...
        <section className="mb-14 relative">
          {/* Main Container - CRITICAL: overflow-visible so dropdown can pop out */}
          <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-indigo-200 flex flex-col lg:flex-row items-center justify-between gap-8 relative z-10 overflow-visible min-h-[300px]">
            
            {/* BACKGROUND LAYER - Handles clipping of blobs while parent is overflow-visible */}
            ...
            <div className="relative z-10 text-center lg:text-left flex-1">
              <h2 className="text-4xl lg:text-5xl font-black mb-3 tracking-tight">Total Syllabus Draw</h2>
            ...
            <div className="relative z-20 flex flex-col sm:flex-row items-stretch gap-4 bg-white/10 p-3 rounded-3xl backdrop-blur-xl border border-white/20 shadow-inner w-full lg:w-auto min-w-[320px] lg:min-w-[500px]">
              ...
              <div className="relative flex-1" ref={dropdownRef}>
                ...
                >
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200 opacity-90 leading-none mb-1.5">Focus Priority:</span>
                ...
                {isPcmDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] z-[100] dropdown-animate border border-slate-100 p-2 overflow-hidden">
                    {(Object.keys(filterLabels) as (keyof typeof filterLabels)[]).map((key) => (
                      <button
                        key={key}
                        onClick={() => { setPcmDrawFilter(key); setIsPcmDropdownOpen(false); }}
                        className={`w-full text-left px-5 py-3.5 text-[11px] font-black uppercase tracking-[0.15em] rounded-2xl transition-all duration-200 mb-1 last:mb-0 ${
                            pcmDrawFilter === key 
              ...
              <button 
                ...
                className="bg-white text-indigo-700 px-10 h-16 rounded-2xl font-black text-lg hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:-translate-y-0.5 transition-all active:scale-95 shadow-xl shadow-black/5 flex items-center justify-center gap-3 whitespace-nowrap"
              >
                Draw from PCM 🍀
              </button>
            </div>
      ...
      <footer className="max-w-7xl mx-auto px-4 mt-20 text-center">
        ...
        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">Keep pushing, you're getting closer every day</p>
