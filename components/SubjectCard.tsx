...
const SubjectCard: React.FC<SubjectCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-visible flex flex-col transition-all hover:shadow-2xl h-[720px] group/card">
      {/* Subject Header */}
        ...
        {isEditing ? (
          <div className="flex flex-col h-full space-y-6">
            <div className="flex justify-between items-center shrink-0">
              <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Editing Mode</h3>
              <button 
                onClick={() => onRestoreDefaults(subject)}
                className="text-[10px] font-black text-indigo-600 hover:text-indigo-800 transition-colors uppercase bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100"
              >
                Reset Default
              </button>
            </div>
            
            <form onSubmit={handleAdd} className="space-y-3 shrink-0 p-5 bg-slate-50 rounded-3xl border border-slate-200/50 shadow-inner">
              <input
                type="text"
                value={newChapterName}
                onChange={(e) => setNewChapterName(e.target.value)}
                placeholder="Chapter Name..."
                className="w-full px-5 py-4 text-sm font-bold border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-slate-900/5 bg-white shadow-sm"
              />
              <div className="flex gap-2">
                <select 
                  value={newChapterPriority}
                  onChange={(e) => setNewChapterPriority(e.target.value as Priority)}
                  className="flex-1 px-5 py-4 text-sm font-bold border border-slate-200 rounded-2xl bg-white outline-none cursor-pointer hover:bg-slate-50 appearance-none"
                >
                ...
                <button 
                  type="submit"
                  className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-sm font-black hover:bg-black transition-all active:scale-95 shadow-lg shadow-black/10"
                >
                  ADD
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto pr-3 custom-scrollbar space-y-2">
              {allChapters.map((chapter) => (
                <div key={chapter.name} className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 group shadow-sm hover:border-slate-300 transition-all">
                  <div className="flex flex-col min-w-0">
                    <span className="text-sm font-bold text-slate-800 truncate">{chapter.name}</span>
                  ...
                  <button 
                    ...
                    className="text-slate-300 hover:text-rose-600 p-2 rounded-xl hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100"
                  >
          ...
          <>
            <div className="flex flex-col gap-4 mb-8 shrink-0 relative z-30">
              <div className="flex items-center justify-between px-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Draw Focus</label>
                <div className="relative" ref={filterRef}>
                    <button 
                        onClick={() => setIsFilterDropdownOpen(!isFilterDropdownOpen)}
                        className="flex items-center gap-3 px-4 py-2 bg-slate-100/80 hover:bg-slate-200/80 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200/50"
                    >
                    ...
                    {isFilterDropdownOpen && (
                        <div className="absolute top-full right-0 mt-2 bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.1)] border border-slate-100 p-1 min-w-[160px] dropdown-animate z-50">
                            {(Object.keys(filterLabels) as (keyof typeof filterLabels)[]).map((key) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                                        drawFilter === key ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500 hover:bg-slate-50'
                                    }`}
                                >
                                    {filterLabels[key]}
                                </button>
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative z-10">
                <div className="flex justify-between items-center mb-5 px-1 shrink-0">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.25em]">Chapter List</h3>
                    <button 
                        onClick={() => onReset(subject)}
                        className="text-[10px] font-black text-slate-300 hover:text-rose-500 transition-colors uppercase tracking-[0.1em]"
                    >
                        RESET PROGRESS
                    </button>
                </div>
                            ...
                            return (
                                <label 
                                    ...
                                    isCompleted 
                                        ...
                                        : 'bg-white border-slate-100 hover:border-slate-800 hover:shadow-lg shadow-sm'
                                    }`}
                                >
                                    <div className={`relative flex items-center justify-center w-7 h-7 rounded-xl border-2 transition-all shrink-0 ${
                                    isCompleted 
                                        ? `border-slate-200 bg-slate-200` 
                                        : `border-slate-200 group-hover/item:border-slate-800`
                                    }`}>
                                    <input
                                        type="checkbox"
                                    ...
                                    <div className={`text-[15px] font-extrabold transition-all truncate leading-tight ${isCompleted ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                                        {chapter.name}
                                    </div>
                                    {!isCompleted && (
                                        <span className={`text-[9px] font-black uppercase tracking-widest mt-1.5 inline-block px-2 py-0.5 rounded-md ${chapter.priority === 'High' ? 'text-rose-600 bg-rose-50' : chapter.priority === 'Medium' ? 'text-amber-600 bg-amber-50' : 'text-indigo-600 bg-indigo-50'}`}>
                                        {chapter.priority}
                                        </span>
                                    )}
                                    </div>
